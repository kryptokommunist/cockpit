 ...
 */
export class FutureFinancialQA {
  ...
  async checkLLMAvailability() {
    try {
      console.log('[FutureFinancialQA] Checking LLM availability...');
      const available = await this.llmService.checkAvailability();
      console.log('[FutureFinancialQA] LLM available:', available);
      if (!available) {
        this.showWarning('AI service is not available. Check if backend is running on port 3001.');
      } else {
        console.log('[FutureFinancialQA] AI service is ready!');
      }
    } catch (error) {
      console.error('[FutureFinancialQA] Error checking availability:', error);
